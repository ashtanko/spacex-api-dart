...
import 'package:data/src/di/data_getit.dart';
    ...
    data.keys.toList().asMap().forEach((index, dynamic key) {
      ...
      if (value is Map) {
        if (compact) {
          ...
          _printPrettyMap(logPrint, value, tabs: mTabs);
        }
        ...
        if (compact) {
          ...
          logPrint('║${_indent(mTabs)} $key: [');
          _printList(logPrint, value, tabs: mTabs);
        ...
        if (msg.length + indent.length > linWidth) {
          ...
          for (var i = 0; i < lines; ++i) {
            logPrint('║${_indent(mTabs)} ${msg.substring(
              i * linWidth,
